...
> Статус: клиент и бэкенд написаны, client ID заведены, бэкенд ВЫКАЧЕН.
> Вход всё ещё не работает по двум причинам: сервер отвечает `503` — не задан
> `app.google.client-ids` (§6), и потерян keystore, чей SHA-1 вписан в Android-клиент (§4).
...
SHA-1 (действующий, ключ EAS, с 02.09.2026):
              0B:74:39:49:34:24:BD:16:CB:71:EF:78:F2:2C:7C:71:75:88:70:CF

SHA-1 (старый, потерянный debug-keystore — можно удалить из клиента):
              7C:DF:FA:38:8C:A2:00:A1:92:48:B9:25:6C:88:2A:C5:60:C3:95:FC
```

⚠️ **У Android-клиента поле SHA-1 ОДНО, второй отпечаток в него не добавить.**
Значение заменяется. Поэтому «отладочный и релизный отпечатки» — это не две
строки в одном клиенте, а **два отдельных Android-клиента** с одинаковым
package name: под ключ разработки и под релизный. Второй понадобится к
публикации в Play, где Google переподписывает приложение своим ключом.

**Отладочный — ⚠️ ПОТЕРЯН (31.08.2026).** Keystore создавали 13.08.2026 стандартными
параметрами Android (`~/.android/debug.keystore`, alias `androiddebugkey`, пароли
`android`), копию клали в `mobile/credentials/`. На текущей машине нет ни того, ни
другого: нет `~/.android`, нет `credentials.json`, нет папки `credentials/`.
...
### ⚠️ Custom URI scheme у Android-клиента выключен по умолчанию

Симптом (02.09.2026, первая APK): Google не доходит до выбора аккаунта и
показывает «Доступ заблокирован: недопустимый запрос от приложения», а в
подробностях —
...
Это НЕ про test users (там был бы `access_denied`) и не про SHA-1. Google по
умолчанию запрещает браузерный OAuth с кастомной схемой
(`uz.uzcasting.app:/oauthredirect`) для Android-клиентов, подталкивая к нативной
библиотеке.

Лечится тумблером: **Credentials → клиент `UzCasting Android` → Advanced
settings → Enable Custom URI scheme → Save.** Применяется от 5 минут до
нескольких часов. Код при этом менять не надо — приложение шлёт корректный
запрос, его просто не принимали.

⚠️ У новых клиентов Google этот тумблер убирает. Если его нет — остаётся переход
на `@react-native-google-signin/google-signin` (провайдер `providers/google` в
`expo-auth-session` объявлен **устаревшим** в SDK 57, документация Expo ведёт
туда же). Это нативный модуль, то есть **обязательно новая сборка**: по воздуху
(`eas update`) такое не доставляется.
...
| 5 | **`app.google.client-ids` на сервере** — иначе эндпоинт отвечает `503` | ✅ добавлено 02.09.2026, эндпоинт отвечает `401` |
| 6 | **Keystore сборки** — ключ отдан EAS, APK собирается | ✅ 02.09.2026, сборка `69939476` |
| 6a | **Новый SHA-1 в Android-клиенте** — `0B:74:39:...:70:CF` (§4) | ✅ вписан 02.09.2026 (у Google применяется от 5 минут до нескольких часов) |
| 7 | **SHA-1 релизного ключа** — когда решим, кто подписывает продакшен (наш keystore или Play App Signing) | ⏳ к этапу публикации |

Всё закрыто 02.09.2026. Остаётся один ограничитель, который снимается только
публикацией: проект в статусе `Testing`, и войти могут **лишь аккаунты из списка
test users** (пункт 2). Для любого другого Google ответит «доступ запрещён», и по
виду это неотличимо от поломки — при расширении круга тестировщиков их сначала
