...
| 6a | **Новый SHA-1 в Android-клиенте** — `0B:74:39:...:70:CF` (§4) | ❌ **блокирует вход**, вписывается в Google Cloud Console |
| 7 | **SHA-1 релизного ключа** — когда решим, кто подписывает продакшен (наш keystore или Play App Signing) | ⏳ к этапу публикации |

Осталось 5 и 6a, и они независимы: первое делает тот, у кого доступ к серверу,
второе — тот, у кого доступ к Google Cloud Console. Пока не сделано 5, человек
дойдёт до выбора аккаунта, приложение получит `id_token`, а обмен упадёт с `503`.
Пока не сделано 6a, Google не отдаст токен этой сборке вообще.
