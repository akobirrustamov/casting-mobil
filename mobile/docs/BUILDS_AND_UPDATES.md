...
EAS собирает под аккаунтом `diyorbekjumayev1` (`app.json` → `owner`), проект
`705e3238-9b8c-4676-9099-556e01afa79b`. Без авторизации `eas build` просто
не начнётся.

⚠️ **С 15.09.2026 проект переехал с `bukakish` на `diyorbekjumayev1`.** У
`bukakish` доступа больше нет, поэтому `owner`, `extra.eas.projectId` и
`updates.url` сменились. Что из этого следует:

- ключ подписи у нового проекта свой, SHA-1
  `6C:70:BF:FE:27:B3:B0:F6:A5:E5:F4:0F:F0:28:CE:00:D6:A4:CE:C9`. Его нужно
  вписать в Android-клиент Google, иначе вход через Google не заработает (§1.2);
- APK из проекта `bukakish` подписаны другим ключом: поверх них новая не
  встанет, старую надо сначала удалить;
- переменные окружения EAS (§2) заведены в новом проекте заново 15.09.2026
  для `preview` и `production`.

Сейчас в `.env.local` лежит токен уже НОВОГО аккаунта: проверено
22.09.2026 — `eas whoami` отвечает `diyorbekjumayev1 (authenticated using
EXPO_TOKEN)`. То есть ассистенту `eas login` не нужен, достаточно
экспортировать токен (как ниже).
...
Вписан 02.09.2026. ⚠️ У Google изменение применяется от 5 минут до нескольких
часов — сразу после сохранения вход может ещё не работать.

### 3.4. ⚠️ Чем на самом деле управляется сервис

`deploy/uzcasting.service` описывает systemd, но на боевом сервере **юнита с
таким именем нет** — `systemctl restart uzcasting` отвечает `Unit not found`.
Приложение живёт под **supervisor**:

```sh
sudo supervisorctl restart uzcasting
```

Файл `uzcasting.service` в `/root` — просто копия из репозитория, её не
устанавливали. Знать это важно: Spring читает `application.properties` только
при старте, и правка конфига без перезапуска выглядит как «настройка не
применилась».
