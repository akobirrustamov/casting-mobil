...
переменные в бандл; токен от аккаунта не должен уезжать в APK. Проверить,
что `.env.local` в гитигноре, — обязательно.

Токен даёт право собирать и публиковать обновления от вашего имени. Отозвать
можно на той же странице.

### 1.2. Решение по ключу подписи

Разобрано в [GOOGLE_AUTH.md §4](./GOOGLE_AUTH.md). Коротко: старый
debug-keystore потерян, а профили `development` и `preview` стоят на
`"credentialsSource": "local"` и ищут `credentials.json`, которого нет.
**Сборка не стартует.**

Развилка одна из двух:

| Вариант | Что сделать | Цена |
|---|---|---|
| Найти старый ключ | положить `credentials/` и `credentials.json` на место | ничего не меняется |
| Отдать ключ EAS | убрать `credentialsSource` из обоих профилей | новый SHA-1 надо вписать в Android-клиент Google, иначе вход через Google в этой APK не заработает |

Второй вариант надёжнее: ключ хранится на стороне EAS и больше не теряется.
Новый отпечаток покажет `eas credentials -p android`.
