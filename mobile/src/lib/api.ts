 ...
 *
 * ⚠️ Домен без TLS. Токен сессии, refresh-токен, номер телефона и код из
 * SMS идут по сети открытым текстом — в одной Wi-Fi сети их можно прочитать
 * и войти в чужой аккаунт. Для тестовой сборки внутри команды это принято
 * осознанно; до реальных пользователей домену нужен сертификат, и тогда
 * здесь меняется одна буква — `https`.
 */
const DEFAULT_API_URL = 'http://uzcasting.com';
