...
export function ScreenState({
  ...
  if (kind === 'loading') {
    return (
      /**
       * ⚠️ Подложка ПОЛУПРОЗРАЧНАЯ, и это починка, а не вкус.
       *
       * Была сплошная `bg-ink`. На экране целиком это незаметно — фон и
       * так `ink`. А внутри чего-то светлее (шторка донатов, карточка)
       * она вырезала явный тёмный прямоугольник вокруг знака: заказчик
       * прислал ровно такой скриншот 10.09.2026.
       *
       * Полупрозрачная затемняет то, что под ней, вместо того чтобы
       * закрашивать своим цветом, — и на любой поверхности выглядит
       * затемнением, а не заплаткой.
       *
       * Отступ маленький: у загрузчика уже есть собственное поле под ореол.
       */
      <View className="flex-1 items-center justify-center gap-1 bg-ink/75">
        <BrandLoader />
