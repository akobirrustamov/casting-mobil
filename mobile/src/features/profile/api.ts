...
/**
 * Удаление аккаунта — `DELETE /api/v1/app/me` (13.09.2026).
 *
 * <h2>Зачем это есть</h2>
 * Требование Google Play: если в приложении можно завести аккаунт, должен
 * быть и способ его удалить — внутри приложения и по веб-ссылке. Без
 * этого приложение в магазин просто не пускают.
 *
 * <h2>⚠️ Что происходит на сервере</h2>
 * Строка пользователя НЕ удаляется — к ней привязаны платежи, которые
 * нужно хранить. Чистятся телефон, email, имя, аватар; отзываются все
 * сессии; удаляются устройства, избранное и прогресс просмотра.
 * Комментарии остаются, автор становится «удалённым пользователем».
 *
 * Телефон освобождается: с него можно зарегистрироваться заново.
 */
export async function deleteAccount(): Promise<void> {
  await api.delete('/api/v1/app/me');
}

/**
 * ⚠️ Сессию гасит ВЫЗЫВАЮЩИЙ, а не этот хук.
 *
 * После удаления токен уже недействителен, и любой следующий запрос
 * получит 401. Если разлогинить здесь, экран под нами успеет перерисоваться
 * на «войдите» до того, как покажется подтверждение — человек решит, что
 * его просто выкинуло, и не поймёт, удалился ли аккаунт.
 */
