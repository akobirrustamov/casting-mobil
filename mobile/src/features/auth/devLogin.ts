 ...
 *
 * <h2>Зачем</h2>
 * Google-вход требует dev build: `expo-auth-session` строит redirect как
 * `${applicationId}:/oauthredirect`, а в Expo Go `applicationId` — это
 * `host.exp.exponent`, который в Google Cloud не зарегистрирован. Вход по
 * номеру упирается в Eskiz, которого на локальном контуре нет. То есть
 * зайти в приложение и посмотреть внутренние экраны было нечем.
 *
 * <h2>Что это НЕ</h2>
