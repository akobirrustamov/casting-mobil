...
export type ContentCard = {
  ...
  /**
   * Сколько раз открывали. `null` — старая сборка бэкенда поля не отдаёт.
   *
   * ⚠️ `null` и `0` — разные вещи, как и в `watch/types.WatchInfo`: ноль
   * прислал сервер, и это факт; `null` значит «не спрашивали». Рисовать
   * вместо него «0 просмотров» — выдумывать цифру на КАЖДОЙ карточке.
   *
   * ⚠️ «Нравится» на карточке НЕ РИСУЕТСЯ. Карточка в ряду шириной
   * ~105px, и в нижнем углу кадра уже стоит длительность — второй
   * счётчик туда не влезает. Число приходит ниже, в `likeCount`, — для
   * страницы контента, а не для карточки.
   */
  viewCount: number | null;

  /**
   * «Нравится» — ДЛЯ СТРАНИЦЫ КОНТЕНТА, на карточке не рисуется.
   *
   * ⚠️ Зачем (10.09.2026, «лайки приходят слишком медленно»): на страницу
   * заходят нажатием на эту самую карточку, то есть число уже лежит в
   * телефоне. Сердце рисуется с ним сразу, а ответ `/content/{id}`
   * потом только уточняет. `null` — старая сборка бэкенда поле не
   * присылает; тогда число придёт, как раньше, со страницей.
   */
