...
function Loaded({ info, query }: { info: WatchInfo; query: WatchQuery }) {
  ...
  /**
   * ⚠️ Здесь видео начинается СРАЗУ, и это не противоречит правилу
   * «только по кнопке».
   *
   * Правило про карточку контента: там человек ещё выбирает. Сюда он
   * попадает, уже нажав серию в списке — кнопка была ею. Ещё одна
   * кнопка на этом экране означала бы два нажатия подряд ради одного
   * действия.
   */
  return (
    <Screen
