...
// Стор сюда не ходит — модуль подменяется только потому, что его
// тянет `auth/store`, а поднимать настоящий axios в этих проверках
// незачем. Разрешение на запись живёт в `WRITE_ALLOWLIST` и
// проверяется в `src/lib/__tests__/api.test.ts`.
jest.mock('@/lib/api', () => ({
